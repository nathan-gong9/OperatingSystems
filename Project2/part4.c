...
typedef struct {
    pid_t pid;
    bool is_running; // True if the process is currently running, false if it has terminated
} ProcessInfo;
...
void print_process_info(ProcessInfo process) {
    ...
    // Read I/O statistics
    snprintf(path, sizeof(path), "/proc/%d/io", process.pid);
    file = fopen(path, "r");
    ...
    // Print process info
    printf("%-5d | %-10.2f | %-10d | %-15ld | %-15ld | %-10s\n", 
           process.pid, cpu_time, mem_usage, rchar, wchar, 
           process.is_running ? "Running" : "Terminated");
}

void update_process_info(ProcessInfo *processes, int num_processes) {
    printf("PID   | CPU Time (s) | Memory (KB) | I/O Read (Chars) | I/O Write (Chars) | Status\n");
...
void alarm_handler(int sig) {
    if (kill(processes[current_process].pid, SIGSTOP) == -1) {
    ...
    if (kill(processes[current_process].pid, SIGCONT) == -1) {
        ...
        if (strcmp(argv[1], "-f") == 0) {
            ...
            while ((nread = getline(&line, &len, workload)) != -1) {
                ...
                if (pid < 0) {
                    ...
                    processes[line_number].pid = pid;
                    processes[line_number].is_running = true; // Mark as running
                    line_number++;
                }
            }
            
            for (int i = 0; i < num_processes; i++) {
                if (kill(processes[i].pid, SIGUSR1) == -1) {
            ...
            if (kill(processes[0].pid, SIGCONT) == -1) {
            ...
            while (num_processes > 0) {
                ...
                if (pid > 0) {
                    ...
                    // Find and mark the terminated process
                    for (int i = 0; i < num_processes; i++) {
                        if (processes[i].pid == pid) {
                            processes[i].is_running = false; // Mark as terminated
                            break;
                        }
                    }
            ...
            update_process_info(processes, num_processes); // Final update to show terminated processes
